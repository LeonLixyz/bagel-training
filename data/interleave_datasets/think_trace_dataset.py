...
class ThinkTraceJSONLIterableDataset(InterleavedBaseIterableDataset, DistributedIterableDataset):
    ...
    def parse_row(self, json_line):
        ...
        # 3. Interleave text parts and images from reasoning trace
        ...
        for i, text_part in enumerate(text_parts):
            ...
            if text_part.strip():
                # If this text part is followed by an image, the im_end token should predict vision_start
                next_token_label = self.start_of_image if i < len(loaded_images) else None
                data = self._add_text(data, text_part.strip(), need_loss=True, enable_cfg=True, next_token_label=next_token_label)
