...
def main():
    ...
    parser.add_argument("--output_model", type=str, default=None,
                        help="Path to save bf16 model (auto-generated in checkpoint dir if not provided)")
    parser.add_argument("--token", type=str, default=None,
                        help="Hugging Face token")
    parser.add_argument("--convert_only", action="store_true",
                        help="Only convert to bf16, don't upload")
    parser.add_argument("--cleanup", action="store_true",
                        help="Remove bf16 file after upload")
    ...
    if args.repo_id is None:
        args.repo_id = f"vlm-reasoning-cot/Text-only-ckpt-{checkpoint_formatted}"
    
    if args.checkpoint_dir is None:
        args.checkpoint_dir = f"/dev/shm/results/checkpoints/{checkpoint_formatted}"
    
    if args.input_model is None:
        args.input_model = f"/dev/shm/results/checkpoints/{checkpoint_formatted}/model.safetensors"
    
    # Auto-generate output path in the same directory as the checkpoint
    if args.output_model is None:
        args.output_model = os.path.join(args.checkpoint_dir, "model_bf16.safetensors")
    ...
    if not args.convert_only:
        ...
        # Optionally clean up the bf16 file after upload
        if args.cleanup:
            try:
                os.remove(bf16_model_path)
                print(f"✓ Cleaned up local file: {bf16_model_path}")
            except Exception as cleanup_error:
                print(f"Warning: Failed to remove {bf16_model_path}: {cleanup_error}")
    else:
        print("Conversion completed. Skipping upload (--convert_only flag used)")
        print(f"BF16 model saved at: {bf16_model_path}")
